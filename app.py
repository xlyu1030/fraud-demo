# Overwrite app.py with Modified Theft Rule 1 Logic
code = """
...
with tab3:
    ...
    # 1. Define Rules (UPDATED RULE 1 LOGIC)
    # Rule 1: Score>=500 & Time<=1000 & FailLog>=1 & FailTxn>=1
    t1_c1 = (df['model_score'] >= 500)
    t1_c2 = (df['time_on_file'] <= 1000)
    t1_c3 = (df['failed_logins_24h'] >= 1) # Changed from <1
    t1_c4 = (df['failed_transactions'] >= 1) # Changed from <1
    ...
    # 4. Playgrounds with FULL Metrics
    ...
    with col_t1a:
        ...
        use_t1_c3 = st.checkbox("Fail Logins (High)", True, key="t1_c3") # Changed Label
        use_t1_c4 = st.checkbox("Fail Txn (High)", True, key="t1_c4") # Changed Label
        st.markdown("---")
    ...
    with col_t1b:
        ...
        if use_t1_c3: mask_t1 &= (df['failed_logins_24h'] >= val_t1_fail) # Changed to >=
        if use_t1_c4: mask_t1 &= (df['failed_transactions'] >= val_t1_ftxn) # Changed to >=
...
# ==============================================================================
# TAB 4: MANAGER SIMULATOR (Existing)
# ==============================================================================
with tab4:
...
print("app.py updated with Modified Theft Rule 1 Logic.")
